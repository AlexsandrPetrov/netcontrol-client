...
def generate_control_file(version):
    ...
    content = """Package: netcontrol
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: NetControl <noreply@tehalex.ru>
Homepage: https://tehalex.ru
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/netcontrol')
    system2('mkdir -p tmpdeb/etc/netcontrol/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/netcontrol/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/netcontrol || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/netcontrol/')
    system2(
        'cp ../res/netcontrol.service tmpdeb/usr/share/netcontrol/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/netcontrol.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/netcontrol.svg')
    system2(
        'cp ../res/netcontrol.desktop tmpdeb/usr/share/applications/netcontrol.desktop')
    system2(
        'cp ../res/netcontrol-link.desktop tmpdeb/usr/share/applications/netcontrol-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/netcontrol/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/netcontrol/')
    system2(
        'cp ../res/pam.d/rustdesk.debian tmpdeb/etc/pam.d/netcontrol')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/netcontrol/files/polkit && chmod a+x tmpdeb/usr/share/netcontrol/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb netcontrol.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('netcontrol.deb', '../netcontrol-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/netcontrol || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/netcontrol/')
    system2(
        'cp ../res/netcontrol.service tmpdeb/usr/share/netcontrol/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/netcontrol.png')
...
def main():
    ...
    '''.format(pa, version))
                ...
                system2(
                    'cp res/netcontrol-link.desktop tmpdeb/usr/share/applications/netcontrol-link.desktop')
                os.system('mkdir -p tmpdeb/etc/netcontrol/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/netcontrol/')
                os.system('mkdir -p tmpdeb/etc/X11/rustdesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/rustdesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/rustdesk.debian tmpdeb/etc/pam.d/rustdesk')
                system2('strip tmpdeb/usr/bin/netcontrol')
                system2('mkdir -p tmpdeb/usr/share/netcontrol')
                system2('mv tmpdeb/usr/bin/netcontrol tmpdeb/usr/share/netcontrol/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/netcontrol/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb netcontrol.deb; /bin/rm -rf tmpdeb/')
