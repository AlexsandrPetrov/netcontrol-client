...
const borderColor = Color(0xFF2A5FE0);
...
class _DesktopHomePageState extends State<DesktopHomePage>
  ...
  Widget buildHelpCards(String updateUrl) {
    // Ребрендинг: карточка "доступна новая версия" со ссылками на
    // rustdesk.com/download и на релизы upstream убрана целиком. Клиент
    // раздаётся через нашу же систему, а не скачиванием с сайта RustDesk,
    // так что это предложение вело бы пользователя не туда. Остальные
    // карточки ниже - не реклама, а рабочие предупреждения (нет прав,
    // не установлена служба, Wayland), их оставляем.
    ...
    if (isWindows && !bind.isDisableInstallation()) {
      ...
      if (bind.isSelinuxEnforcing()) {
        ...
        if (bind.mainGetLocalOption(key: keyShowSelinuxHelpTip) != 'N') {
          // Ребрендинг: предупреждение оставлено, кнопка "Help" со ссылкой на
          // rustdesk.com убрана - наружу из нашего клиента вести некуда.
